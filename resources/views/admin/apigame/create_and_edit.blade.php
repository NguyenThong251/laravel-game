...
// $title = $isUpdated?"API游戏修改":"API游戏Added";
@endphp
            ...
            <form method="post" class="form-horizontal"
                ...
                @foreach(config('platform.language_type') as $k => $v)
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <input type="text" required class="form-control" name="lang_json[{{ $k }}]" placeholder="Please enter 游戏title[{{$v}}]"
                ...
                <div class="form-group">
                    <label class="col-sm-2 control-label ">PC video page logo</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="web_logo"
                            value="{{ $isUpdated?$model->web_logo:"" }}" @if(!$isUpdated) required @endif>
                    </div>
                    <div class="col-sm-4">
                        <span class="help-block">Only used on the PC video page</span>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-2 control-label">@lang('res.common.upload_image')</label>
                    <div class="col-sm-8">
                        <ul class="list-inline clearfix lyear-uploads-pic" id="web_logo"
                            data-upload-url="{{ route('attachment.upload',['file_type' => 'pic','category' => 'api_game']) }}"
                            data-delete-url="{{ route('attachment.delete') }}" @if($isUpdated)
                            data-image-url="{{ $model->web_logo }}" @endif>
                        </ul>
                    </div>
                </div>				
                <div class="form-group">
...
<script>
    $(function () {
        ...
        // 单图上传
        $('#web_logo').imageUpload({
            $callback_input: $('.form-control[name="web_logo"]'),
            showErrorDialog: $.utils.layerError,
            showSuccessDialog: $.utils.layerSuccess
        });		
        // 单图上传
