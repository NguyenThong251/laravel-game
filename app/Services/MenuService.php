...
class MenuService
{
    ...
    public function getPermissionsByGuard($guard = "web")
    {
        ...
        // return $data;
        // Lấy người dùng từ guard đã chỉ định
        $user = request()->user($guard);

        // Nếu không có người dùng, có thể ném ra một ngoại lệ tùy ý hoặc trả về một mảng rỗng
        if (!$user) {
            return []; // Hoặc ném ngoại lệ tùy thuộc vào logic của bạn
        }

        // Lấy danh sách quyền từ người dùng mà không cần xác thực
        $permission_ids = $user->getPermissionsViaRoles()
            ->where("is_show", true)
            ->where("guard_name", $guard)
            ->where("level", 0)
            ->pluck('id');

        // Truy vấn Permission theo danh sách quyền đã lấy
        $data = Permission::whereIn('id', $permission_ids)->get();
